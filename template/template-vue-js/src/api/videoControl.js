...
export const setPTZControl = async (option, direction, stop) => {
  ...
  const data = !stop ? `<?xml version='1.0' encoding='UTF-8'?><PTZData><pan>${direction === 'right' ? '10' : direction === 'left' ? '-10' : '0'}</pan><tilt>${direction === 'top' ? '10' : direction === 'bottom' ? '-10' : '0'}</tilt></PTZData>` : `<?xml version='1.0' encoding='UTF-8'?><PTZData><pan>0</pan><tilt>0</tilt></PTZData>`
